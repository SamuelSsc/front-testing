...
test.describe("Home Page E2E Tests", () => {
  ...
  test("should add a new task", async ({ page }) => {
    ...
    await expect(
      page.locator('input[placeholder="Descreva sua tarefa"]')
    ).toHaveValue("");
  ...
  test("should have no tasks", async ({ page }) => {
    const noTasksMessage = page.locator(
  ...
  test("should mark a task as completed and reorder", async ({ page }) => {
    ...
    const task1Checkbox = page.locator(
      `.todo-list li:has-text("${task1}") input[type="checkbox"]`
    );
  ...
  test("should remove a unique task", async ({ page }) => {
    ...
    await page
      .locator(`.todo-list li:has-text("${task}")`)
      .locator('[aria-label="Deletar Tarefa"]')
      .click();
  ...
  test("should remove task and keep others", async ({ page }) => {
    ...
    await page
      .locator(`.todo-list li:has-text("${taskWrong}")`)
      .locator('[aria-label="Deletar Tarefa"]')
      .click();

    await expect(
      page.locator(`.todo-list li:has-text("${taskWrong}")`)
    ).not.toBeVisible();
    await page.waitForTimeout(500);
